...
matrix normalize(matrix x, matrix m, matrix v, int groups)
{
    ...
    for (i = 0; i < x.rows; ++i) {
        for (j = 0; j < x.cols; ++j) {
            ...
            norm.data[i * x.cols + j] /= sqrt(v.data[j / n] + eps);
        }
    }
...
matrix delta_batch_norm(matrix d, matrix dm, matrix dv, matrix m, matrix v, matrix x)
{
    ...
    for (i = 0; i < d.rows; ++i) {
        for (j = 0; j < d.cols; ++j) {
            ...
            dx.data[i * d.cols + j] += dv.data[j / n] * 2 * (x.data[i * d.cols + j] - m.data[j / n]) / d.rows / n;
            dx.data[i * d.cols + j] += dm.data[j / n] / d.rows / n;
        }
    }
    return dx;
}
